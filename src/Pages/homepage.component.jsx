...
const HomePage = () => {
  ...
  const searchValue = async (input) => {
    ...
    if (sporingsData.consignmentSet[0].error) {
      setDataFetchSuccess(true); // i'm setting it to true even if the request fails for debugging purpose
  ...
  return (
    <main className="grid-container">
      <section className="main-left">
        <Search msg={searchValue ? 'TESTPACKAGE-AT-PICKUPPOINT' : errorMsg} searchValue={searchValue} />
      </section>
